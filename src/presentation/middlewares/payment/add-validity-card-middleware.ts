...
export class AddValidityCardMiddleware implements Middleware {
  constructor(private readonly addValidityCard: AddValidityCard) {}

  async handle(httpRequest: HttpRequest, next: Function): Middleware.Result {
    try {
      const result = await this.addValidityCard.add({
        ...httpRequest.body,
