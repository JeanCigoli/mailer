...
export class AddSecurityCodeMiddleware implements Middleware {
  constructor(private readonly addSecurityCode: AddSecurityCode) {}

  async handle(httpRequest: HttpRequest, next: Function): Middleware.Result {
    try {
      const result = await this.addSecurityCode.add({
        ...httpRequest.body,
