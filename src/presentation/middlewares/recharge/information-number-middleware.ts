...
export class InformationNumberMiddleware implements Middleware {
  constructor(private readonly informationNumber: InformationNumber) {}

  async handle(httpRequest: HttpRequest, next: Function): Middleware.Result {
    try {
      const result = await this.informationNumber.check({
        ...httpRequest.body,
